Base.@kwdef struct Params
    # Run Settings
    time_init::Real = -8.0e5           # [yr] Starting time (model years)
    ...
    # Run parameters 
    ## Switches
    active_iso::Bool = false           # Switch: include active isostatic rebound?
    ...
    insol_case::String = "artificial"  # Insolation case: "constant", "artificial", "laskar", "ISI", "caloric", "input"
    ...
    ablation_case::String = "PDD-LIN"  # Surface melting case: "PDD", "ITM", "PDD-LIN"
    ...
    Ppre::Real = 0.6                   # Power of precession (normalised to At)
    taupre::Real = 23e3                # [yr] Precession period
    Pobl::Real = 0.4                   # Power of obliquity (normalised to At)
    tauobl::Real = 41e3                # [yr] Obliquity period
    Pexc::Real = 0.0                   # Power of excentricity (normalised to At)
    tauexc::Real = 100e3               # [yr] Excentricity period
    At::Real = 15.0                    # [ºC or K] Amplitude of temperature forcing (surface temperatures)
    ...
    sref::Real = 0.2                   # [m/yr] Reference Accumulation for northern hemisphere (sref = smean + ka * ΔTmean)
    ks::Real = 0.0015                  # [m/yr/K] sensitivity parameter of snowfall accumulation to temperature (Clasuius clapeyron like) ! 0.004 the default?

    ### ȧ, ablation
    lambda::Real = 0.2                 # [m yr⁻¹ K⁻¹] Proportionality between positive temperatures and surface melt
