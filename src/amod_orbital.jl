...
function calc_artificial_insolation(now_o, par_o)
    # define weights (0 or 1, select the option)
    ...
    return wo * par_o["P_obl"] * cos(2.0*pi*now_o["time"] / par_o["tau_obl"]) +
...
@doc """
    ...
    modifications by Marisa Montoya and Sergio Pérez-Montero 
"""
function calc_laskar_insolation()
...
function calc_insol_day(now_o, par_o)
    if par_o["ins_case"] == "artificial"
        error("ERROR, insolation option not implemented yet")
